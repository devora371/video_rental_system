...
namespace VideoRentalSystem
{
    public partial class RentalForm : Form
    {
        ...
        {
            SqlMethod.loadStat(movieLbl,customerLbl, custLbl, incomeLbl,videoLbl);
        ...
        private void deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            else if (dataGV.Tag.ToString() == "Video")
            {
                ...
                {
                    SqlMethod.Delete(titleTxt, genreTxt, costTxt, languageTxt, copiesTxt, yearTxt, id.ToString());
                }
            ...
            else if (dataGV.Tag.ToString() == "Customer")
            {
                if (nameTxt.Text != "" && id != -1)
                {
                    SqlMethod.Delete(nameTxt, cnctTxt, addTxt, jdateTxt, id.ToString());
        ...
        private void videoSave_Click(object sender, EventArgs e)
        {
            if (titleTxt.Text != "" && genreTxt.Text != "" && languageTxt.Text != "" && costTxt.Text != "")
            {
                ...
                {
                    SqlMethod.Add(titleTxt, genreTxt, costTxt, languageTxt, copiesTxt, yearTxt);
                    id = -1;
                }
                else
                {
                    if (id != -1)
                    {
                        SqlMethod.Update(titleTxt, genreTxt, costTxt, languageTxt, copiesTxt, yearTxt, id.ToString());
                        id = -1;
                    }
                }
                videoBtn.PerformClick();
            ...
            if (customerBtn.Text != "Select Customer")
            {
                if (bookSaveBtn.Text == "Return")
                {
                    if (id != -1)
                    {
                        ...
                        if (a == 0)
                            a = cost;
                        SqlMethod.Update(customerBtn, titleBtn, startPK, endPK, id.ToString(), a);
                        customerBtn.Text = "Select Customer";
                        titleBtn.Text = "Select Video";
                        startPK.Value = DateTime.Now;
                        endPK.Value = DateTime.Now;
                ...
                else
                {
                    if (customerBtn.Text != "Select Customer")
                    {
                        SqlMethod.Add(customerBtn, titleBtn, startPK, endPK);
                        customerBtn.Text = "";
                        titleBtn.Text = "";
                        startPK.Value = DateTime.Now;
                        endPK.Value = DateTime.Now;
        ...
        private void custSaveBtn_Click(object sender, EventArgs e)
        {
            if (nameTxt.Text != "" && cnctTxt.Text != "" && addTxt.Text != "")
            {
                ...
                {
                    SqlMethod.Add(nameTxt, cnctTxt, addTxt, jdateTxt);
                    id = -1;
                }
                else
                {
                    SqlMethod.Update(nameTxt, cnctTxt, addTxt, jdateTxt, id.ToString());
                    id = -1;
                }
                custBtn.PerformClick();
            }
